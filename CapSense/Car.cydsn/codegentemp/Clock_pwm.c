...
void Clock_pwm_Start(void) 
{
    ...
    Clock_pwm_CLKEN |= Clock_pwm_CLKEN_MASK;
}
...
/*******************************************************************************
* Function Name: Clock_pwm_StopBlock
********************************************************************************
* Summary:
...
{
    if (Clock_pwm_CLKEN & Clock_pwm_CLKEN_MASK)
    {
        ...
        CLK_DIST_AMASK = Clock_pwm_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_pwm_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(Clock_pwm_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_pwm_DIV_PTR, oldDivider);
#endif
    }
}
#endif
...
/*******************************************************************************
* Function Name: Clock_pwm_StandbyPower
********************************************************************************
* Summary:
...
void Clock_pwm_StandbyPower(uint8 state) 
{
    if(state == 0)
    {
        Clock_pwm_CLKSTBY &= ~Clock_pwm_CLKSTBY_MASK;
    }
    else
    {
        Clock_pwm_CLKSTBY |= Clock_pwm_CLKSTBY_MASK;
...
/*******************************************************************************
* Function Name: Clock_pwm_SetDividerRegister
********************************************************************************
* Summary:
...
void Clock_pwm_SetDividerRegister(uint16 clkDivider, uint8 restart) 
{
    ...
    uint8 currSrc = Clock_pwm_GetSourceRegister();
    uint16 oldDivider = Clock_pwm_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_pwm_CLKEN & Clock_pwm_CLKEN_MASK;
        ...
        if (currSrc == CYCLK_SRC_SEL_CLK_SYNC_D && (oldDivider == 0 || clkDivider == 0))
        {
            ...
            if (oldDivider == 0 && clkDivider != 0)
            {
                ...
                CY_SET_REG16(Clock_pwm_DIV_PTR, clkDivider);
                Clock_pwm_MOD_SRC &= ~CYCLK_SSS;
            }
            else
            {
...
{
    return CY_GET_REG16(Clock_pwm_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_pwm_SetModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void Clock_pwm_SetModeRegister(uint8 clkMode) 
{
    Clock_pwm_MOD_SRC |= clkMode & Clock_pwm_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_pwm_ClearModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void Clock_pwm_ClearModeRegister(uint8 clkMode) 
{
    Clock_pwm_MOD_SRC &= ~clkMode | ~Clock_pwm_MODE_MASK;
}
...
/*******************************************************************************
* Function Name: Clock_pwm_GetModeRegister
********************************************************************************
* Summary:
...
{
    return Clock_pwm_MOD_SRC & Clock_pwm_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_pwm_SetSourceRegister
********************************************************************************
* Summary:
...
{
    uint16 currDiv = Clock_pwm_GetDividerRegister();
    uint8 oldSrc = Clock_pwm_GetSourceRegister();
    ...
    if (oldSrc != CYCLK_SRC_SEL_CLK_SYNC_D && clkSource == CYCLK_SRC_SEL_CLK_SYNC_D && currDiv == 0)
    {
        ...
        Clock_pwm_MOD_SRC |= CYCLK_SSS;
        Clock_pwm_MOD_SRC =
            (Clock_pwm_MOD_SRC & ~Clock_pwm_SRC_SEL_MSK) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_pwm_GetSourceRegister
********************************************************************************
* Summary:
...
{
    return Clock_pwm_MOD_SRC & Clock_pwm_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_pwm_SetPhaseRegister
********************************************************************************
* Summary:
...
{
    Clock_pwm_PHASE = clkPhase & Clock_pwm_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_pwm_GetPhase
********************************************************************************
* Summary:
...
{
    return Clock_pwm_PHASE & Clock_pwm_PHASE_MASK;
}
