...
/*******************************************************************************
* Function Name: Clock_servo_Start
********************************************************************************
* Summary:
...
void Clock_servo_Start(void) 
{
    ...
    Clock_servo_CLKEN |= Clock_servo_CLKEN_MASK;
}


/*******************************************************************************
* Function Name: Clock_servo_Stop
********************************************************************************
* Summary:
...
/*******************************************************************************
* Function Name: Clock_servo_StopBlock
********************************************************************************
* Summary:
...
{
    if (Clock_servo_CLKEN & Clock_servo_CLKEN_MASK)
    {
...
#else
        CLK_DIST_DMASK = Clock_servo_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(Clock_servo_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_servo_DIV_PTR, oldDivider);
#endif
    }
}
#endif
...
/*******************************************************************************
* Function Name: Clock_servo_StandbyPower
********************************************************************************
* Summary:
...
void Clock_servo_StandbyPower(uint8 state) 
{
    if(state == 0)
    {
        Clock_servo_CLKSTBY &= ~Clock_servo_CLKSTBY_MASK;
    }
    else
    {
        Clock_servo_CLKSTBY |= Clock_servo_CLKSTBY_MASK;
...
/*******************************************************************************
* Function Name: Clock_servo_SetDividerRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void Clock_servo_SetDividerRegister(uint16 clkDivider, uint8 restart) 
{
    uint8 enabled;

    uint8 currSrc = Clock_servo_GetSourceRegister();
    uint16 oldDivider = Clock_servo_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_servo_CLKEN & Clock_servo_CLKEN_MASK;
        ...
        if (currSrc == CYCLK_SRC_SEL_CLK_SYNC_D && (oldDivider == 0 || clkDivider == 0))
        {
            ...
            if (oldDivider == 0 && clkDivider != 0)
            {
                ...
                CY_SET_REG16(Clock_servo_DIV_PTR, clkDivider);
                Clock_servo_MOD_SRC &= ~CYCLK_SSS;
            }
            else
            {
...
/*******************************************************************************
* Function Name: Clock_servo_GetDividerRegister
********************************************************************************
* Summary:
...
{
    return CY_GET_REG16(Clock_servo_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_servo_SetModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void Clock_servo_SetModeRegister(uint8 clkMode) 
{
    Clock_servo_MOD_SRC |= clkMode & Clock_servo_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_servo_ClearModeRegister
********************************************************************************
* Summary:
...
*
*******************************************************************************/
void Clock_servo_ClearModeRegister(uint8 clkMode) 
{
    Clock_servo_MOD_SRC &= ~clkMode | ~Clock_servo_MODE_MASK;
}
...
/*******************************************************************************
* Function Name: Clock_servo_GetModeRegister
********************************************************************************
* Summary:
...
{
    return Clock_servo_MOD_SRC & Clock_servo_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_servo_SetSourceRegister
********************************************************************************
* Summary:
...
{
    uint16 currDiv = Clock_servo_GetDividerRegister();
    uint8 oldSrc = Clock_servo_GetSourceRegister();
    ...
    if (oldSrc != CYCLK_SRC_SEL_CLK_SYNC_D && clkSource == CYCLK_SRC_SEL_CLK_SYNC_D && currDiv == 0)
    {
        ...
        Clock_servo_MOD_SRC |= CYCLK_SSS;
        Clock_servo_MOD_SRC =
            (Clock_servo_MOD_SRC & ~Clock_servo_SRC_SEL_MSK) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_servo_GetSourceRegister
********************************************************************************
* Summary:
...
{
    return Clock_servo_MOD_SRC & Clock_servo_SRC_SEL_MSK;
}
